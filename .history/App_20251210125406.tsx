...
import { getUserProfile, executeTrade, resetAccount, saveUserProfile, toggleWatchlist } from './services/userService';
...
const App: React.FC = () => {
    ...
    const handleTrade = async (symbol: string, price: number, quantity: number, type: 'BUY' | 'SELL') => {
        ...
        if (result.success && result.newProfile) {
            ...
            showToast(`${type} Order Executed!`, 'success');
        } else {
    ...
    const handleToggleWatchlist = async (symbol: string) => {
        if (!user) return;
        const updatedProfile = await toggleWatchlist(user, symbol);
        setUser(updatedProfile);
        const isAdded = updatedProfile.watchlist.includes(symbol);
        showToast(isAdded ? 'Added to Watchlist' : 'Removed from Watchlist', 'info');
    };
    ...
    return (
        <div className={`flex h-screen font-sans ${darkMode ? 'bg-[#020617] text-white selection:bg-[#00d09c]/40' : 'bg-[#F5F7F8] text-gray-900 selection:bg-[#00d09c]/20'}`}>
            ...
            {selectedStock && (
                <StockDetailView 
                    stock={selectedStock} 
                    isWatchlisted={user.watchlist?.includes(selectedStock.symbol) || false}
