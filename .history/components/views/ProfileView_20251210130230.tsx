...
import { Settings, ShieldCheck, RefreshCw, ChevronRight, Save, Camera, MapPin, ChevronLeft, Bell, Lock, UserX, Download, Award, Zap, Layers, TrendingUp } from 'lucide-react';
import { formatCurrency } from '../../utils';
import { saveUserProfile } from '../../services/userService';
import { MOCK_STOCKS } from '../../constants'; // Need stocks to check sectors
...
const ProfileView: React.FC<ProfileViewProps> = ({ user, onViewChange, resetAccount }) => {
    ...
    const calculateAchievements = () => {
        const netWorth = user.balance + user.portfolio.reduce((acc, i) => acc + (i.averagePrice * i.quantity), 0); // Approx net worth based on avg price for simplicity or current if passed, using stored avg for now
        
        // Count unique sectors
        const uniqueSectors = new Set();
        user.portfolio.forEach(item => {
            const stock = MOCK_STOCKS.find(s => s.symbol === item.symbol);
            if(stock) uniqueSectors.add(stock.sector);
        });

        // Check Profitability (Current Value > Invested)
        // Note: Ideally need live prices here, but using Avg Price as proxy for "Holding" logic
        const hasProfit = user.transactions.some(t => t.type === 'SELL' && t.status === 'SUCCESS'); // Simple check: has sold anything? 
        ...
        return [
            {
                ...
                unlocked: user.transactions.length > 0,
            ...
            },
            {
                ...
                icon: TrendingUp,
                title: 'Market Tycoon',
                desc: 'Net Worth > ₹1.5 Lakh',
                unlocked: netWorth > 150000,
            ...
            },
            {
                ...
                icon: Award,
                title: 'Diamond Hands',
                desc: 'Hold 5+ assets simultaneously',
                unlocked: user.portfolio.length >= 5,
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-300 pb-20">
            ...
            {/* Badges & Achievements */}
            <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Badges & Milestones</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {achievements.map(badge => (
                        ...
                        >
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${badge.unlocked ? badge.color : 'bg-gray-200 dark:bg-slate-700 text-gray-400'}`}>
                                <badge.icon size={24} strokeWidth={badge.unlocked ? 2.5 : 2} />
                            </div>
                            <div>
                                <h4 className={`font-bold ${badge.unlocked ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>
                                    {badge.title}
                                </h4>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{badge.desc}</p>
                                {!badge.unlocked && <span className="text-[10px] font-bold uppercase text-gray-400 mt-1 inline-block">Locked</span>}
                            </div>
                        </div>
                    ))}
                </div>
