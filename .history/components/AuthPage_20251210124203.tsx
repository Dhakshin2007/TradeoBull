...
import { ArrowRight, Check, AlertTriangle, User, Mail, Lock, Eye, EyeOff } from 'lucide-react';
...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin, darkMode }) => {
  ...
  if (isSuccess) {
      return (
        <div className={`min-h-screen w-full flex items-center justify-center p-4 ${darkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
            <div className={`max-w-md w-full p-10 rounded-3xl shadow-2xl text-center animate-in zoom-in-95 duration-500 border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-gray-100 text-gray-900'}`}>
                <div className="w-24 h-24 bg-[#00d09c]/20 rounded-full flex items-center justify-center mx-auto mb-8 animate-bounce-slow">
                    <Check size={48} className="text-[#00d09c]" strokeWidth={4} />
                </div>
                <h2 className="text-3xl font-extrabold mb-4 tracking-tight">Welcome Aboard!</h2>
                <p className={`text-lg mb-10 leading-relaxed ${darkMode ? 'text-slate-400' : 'text-gray-500'}`}>
                    Your account has been created successfully. 
                    <br/>Please sign in to start trading.
                </p>
                <button 
