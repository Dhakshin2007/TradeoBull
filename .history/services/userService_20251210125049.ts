...
export const toggleWatchlist = async (currentProfile: UserProfile, symbol: string): Promise<UserProfile> => {
    ...
    if (profile.watchlist.includes(symbol)) {
        profile.watchlist = profile.watchlist.filter(s => s !== symbol);
    } else {
        profile.watchlist.push(symbol);
    }
